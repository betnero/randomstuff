...
while True:  # Infinite loop for script restart or shutdown
    ...
    try:  # try/except for error control
        ...
        ip = input(
            ''''Enter a single IP or IP range in CIDR notation)
             (i.e. 192.168.1.0/24): '''
            )

        # Put network information into a variable. "strict=False" to
        # control ValuError in case the user provides a single IP address
